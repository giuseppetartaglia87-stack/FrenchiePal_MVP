...
export function normalizeLandingVersion(version) {
  return version || 'v4';
}

async function safeInsert(table, row) {
  if (!supabase) return noDatabaseResult();
  const { data, error } = await supabase.from(table).insert([row]).select();
  if (error) console.error(`Insert failed for ${table}:`, error);
  return { data, error };
}
...
async function safeUpsert(table, row, onConflict) {
  ...
  const { data, error } = await supabase
    ...
    .select();
  if (error) console.error(`Upsert failed for ${table}:`, error);
  return { data, error };
}
...
export async function upsertLandingSession({
  ...
  if (!sessionId) return noDatabaseResult();
  const row = {
  ...
  if (typeof leadCaptured === 'boolean') row.lead_captured = leadCaptured;
  return safeUpsert(TABLES.landingSessions, row, 'session_id');
}

export async function markSessionLeadCaptured(args = {}) {
  return upsertLandingSession({ ...args, leadCaptured: true });
}
...
export async function trackEvent({
  ...
  if (!sessionId || !eventName) return noDatabaseResult();
...
export async function saveLead({
  ...
  if (!normalizedEmail) return noDatabaseResult();
...
export async function createChatSession({
  ...
  if (!chatSessionId || !sessionId) return noDatabaseResult();
  const row = {
...
export async function updateChatSession(chatSessionId, updates = {}) {
  if (!supabase || !chatSessionId || Object.keys(updates).length === 0) {
  ...
  const { data, error } = await supabase
    ...
    .select();
  if (error) console.error(`Update failed for ${TABLES.chatSessions}:`, error);
  return { data, error };
}
...
export async function getNextMessageOrder(chatSessionId) {
  if (!supabase || !chatSessionId) return 1;
  const { data, error } = await supabase
    .from(TABLES.chatMessages)
...
export async function saveChatMessage({
  ...
  if (!chatSessionId || !role || !messageText) return noDatabaseResult();
